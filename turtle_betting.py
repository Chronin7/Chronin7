...
def start_race():
    ...
    while contestant_1.xcor() < finish and contestant_2.xcor() < finish and contestant_3.xcor() < finish and contestant_4.xcor() < finish and contestant_5.xcor() < finish:
        contestant_1.forward(random.uniform(5, 20)*contestant_1.speed())
        contestant_2.forward(random.uniform(5, 20)*contestant_1.speed())
        contestant_3.forward(random.uniform(5, 20)*contestant_1.speed())
        contestant_4.forward(random.uniform(5, 20)*contestant_1.speed())
        contestant_5.forward(random.uniform(5, 20)*contestant_1.speed())
        screen.update()
    ...
    amount = [0, 0, 0, 0, 0]
    if balence==0:
        balence,amount=100,[] if get_valid_type(str,"you ran out of money play again (y/n): ","not a valid input",["y","n"],"oh no")=="y" else -1
        if balence==-1:
            quit()
    update_bet_buttons()
def reset():
    ...
    contestant_5.clear()
    contestant_1.penup()
    contestant_1.teleport(-400, 400)
    contestant_2.penup()
    contestant_2.teleport(-400, 300)
    contestant_3.penup()
    contestant_3.teleport(-400, 200)
    contestant_4.penup()
    contestant_4.teleport(-400, 100)
    contestant_5.penup()
    contestant_5.teleport(-400, 0)
    screen.update()
...
def get_valid_type(type_return, prompt, invalid_prompt="Invalid input. Please try again.", valid=None,title="place_bet"):
    while True:
        try:
            to_return = type_return(screen.textinput(title, prompt=prompt))
...
def change_speed():
    global turtle_names
    speed=get_valid_type(int,"what is the speed of the turtles: ","not a valid number",(0,10))
    for x in [contestant_1,contestant_2,contestant_3,contestant_4,contestant_5]:
        x.speed(speed)
...
start_button = button(-50,-380,100,-330,"start",start_race,[],"black","green")
buttons.append(start_button)
reset_button = button(-50,-430,100,-380,"reset",reset,[],"black","red")
buttons.append(reset_button)
speed = button(-50,-480,100,-430,"speed",change_speed,[],"black","blue")
buttons.append(speed)
ref_1.penup()
ref_1.color("red")
ref_1.goto(400, -50)
ref_1.pendown()
