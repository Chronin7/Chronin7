...
nloot = [PlaceHolder, PlaceHolder, PlaceHolder, PlaceHolder, PlaceHolder, PlaceHolder, PlaceHolder]
...
def process_position(game_state: GameState):
    ...
    # TODO: FIX THIS
    if "i" in tile:
        only_get_one(int(tile[2:]),game_state,False)
    if "ni" in tile:
        only_get_one(int(tile[2:]),game_state,True)
    if "h" in tile:
